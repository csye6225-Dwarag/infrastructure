...
resource "aws_db_instance" "rdsDbInstance" {
  ...
  kms_key_id              = "${aws_kms_key.res-rds-kms-key-aws.arn}"
...
resource "aws_launch_configuration" "asg_launch_config" {
  ...
  user_data = <<-EOF
        ...
        sudo echo "export snstopic=${aws_sns_topic.EmailNotificationRecipeEndpoint.arn}" >> /etc/environment      
        EOF
...
resource "aws_kms_key" "res-ebs-kms-key-aws" {
  description             = "EBS KMS Key encryption"
policy = <<EOF
{
  "Version": "2012-10-17",
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
          "AWS": "arn:aws:iam::221786680706:root"
      },
      "Action": "kms:*",
      "Resource": "*"
    },
    {
      ...
      "Principal": {
          "AWS": [
              "arn:aws:iam::221786680706:role/aws-service-role/autoscaling.amazonaws.com/AWSServiceRoleForAutoScaling"
          ]
      },
      "Action": [
          "kms:Encrypt",
...
resource "aws_kms_key" "res-rds-kms-key-aws" {
  description             = "Primary RDS Key"
  deletion_window_in_days = 10
}

// resource "aws_kms_alias" "res-ebs-kms-key-aws-alias" {
//   name          = "alias/ebsKey"
//   target_key_id = aws_kms_key.res-rds-kms-key-aws.key_id
// }
...
resource "aws_ebs_default_kms_key" "ebs_encryption_kms_key" {
  key_arn = aws_kms_key.res-ebs-kms-key-aws.arn
}
...
resource "aws_iam_role_policy_attachment" "attachDynamoDbPolicyToRoleEC2" {
  role       = aws_iam_role.EC2-CSYE6225.name
  policy_arn = aws_iam_policy.dynamoDbEc2Policy.arn
}

resource "aws_iam_role_policy_attachment" "attachSNSPolicyToRoleEC2" {
  role       = aws_iam_role.EC2-CSYE6225.name
  policy_arn = aws_iam_policy.topic_policy.arn
}
...
resource "aws_iam_role_policy_attachment" "sns_topic_attach" {
  role       = aws_iam_role.CodeDeployEC2ServiceRole.name
  policy_arn = aws_iam_policy.sns_topic_policy.arn
}
